...
class create_gif():
    def select_images_and_create(self,image_locations,gif_save_location,person):
        ...
        self.create(images,gif_save_location)
    def create(image_locations,gif_save_location):
        try:
            images=[]
            gif_save_location="Gifs//"+gif_save_location+".gif"
            mimwrite(gif_save_location,images,fps=30)
...
class create_graphs():#need to finish
    def line(self,x_title,y_title,y_values,save_location,colour="blue"):#colour RGB values in array
        x_values=[]
        increment=0
        for z in y_values:
